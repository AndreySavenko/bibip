...
if __name__ == '__main__':  
    test = TestCarServiceScenarios()
    tmp_dir = 'my_test'
    # test.test_add_new_car(tmpdir=tmp_dir, car_data=car_data(), model_data=model_data()) # ПРОЙДЕНО
    # test.test_sell_car(tmpdir=tmp_dir, car_data=car_data(), model_data=model_data()) # ПРОЙДЕНО
    # test.test_list_cars_by_available_status(tmpdir=tmp_dir, car_data=car_data(), model_data=model_data()) # ПРОЙДЕНО
    # test.test_list_full_info_by_vin(tmpdir=tmp_dir, car_data=car_data(), model_data=model_data()) # ПРОЙДЕНО
    # test.test_update_vin(tmpdir=tmp_dir, car_data=car_data(), model_data=model_data()) # ПРОЙДЕНО
    # test.test_delete_sale(tmpdir=tmp_dir, car_data=car_data(), model_data=model_data())
    test.test_top_3_models_by_sales(tmpdir=tmp_dir, car_data=car_data(), model_data=model_data())
    print('ok')